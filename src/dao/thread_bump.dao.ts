...
export class ThreadBumpDAO {
  ...
  async exists(threadId: string): Promise<boolean> {
    const res = await query('SELECT 1 FROM thread_bumps WHERE thread_id = $1', [threadId]);
    return (res.rowCount ?? 0) > 0;
  }

  async get(threadId: string): Promise<BumpThreadRow | null> {
    const res = await query<BumpThreadRow>('SELECT * FROM thread_bumps WHERE thread_id = $1', [
      threadId,
    ]);
  ...
  async delete(threadId: string): Promise<boolean> {
    const res = await query('DELETE FROM thread_bumps WHERE thread_id = $1', [threadId]);
    return (res.rowCount ?? 0) > 0;
  }
