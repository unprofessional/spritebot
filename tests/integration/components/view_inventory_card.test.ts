...
describe('view_inventory_card', () => {
  test('renders inventory in pages with quantity labels and pagination controls', () => {
    ...
    expect(view.embeds[0].description).not.toContain('Description 1');
    ...
    expect(JSON.stringify(view.components[1])).toContain('View/Edit an inventory item');
    expect(JSON.stringify(view.components[1])).toContain('Item 1');
    expect(JSON.stringify(view.components[1])).not.toContain('Description 1');
