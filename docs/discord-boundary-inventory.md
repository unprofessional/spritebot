...
1,750ms acknowledgement budget, but its source-level direct calls remain for its later mutation
batch. The first Task 6 batch adds narrow responder contexts for four consistently ephemeral core
commands and a synchronous public/ephemeral policy resolver for `/roll`. Every migrated outbound
operation must name its timeout, total budget, and retry policy.

## Migration Matrix

`Retry-safe?` describes the intended policy, not current automatic retry behavior. The completed
core batch is recorded below; all other source call sites remain pending migration.
...
| Slash/context commands                                        | `src/commands/*.ts` (all 28 command files; 106 findings)                                                                                                                                                                                                  | Interaction replies, deferrals, edits, follow-ups, and modal acknowledgements                  | Yes                                                   | No callback retries                                                                     | Interaction responder                                                  | Core batch complete for `/list-characters`, `/list-games`, `/view-character`, `/view-game`, and `/roll`; remaining batches in Task 6 |
