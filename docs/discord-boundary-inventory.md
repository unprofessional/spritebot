...
`/create-character` now executes behind an ephemeral, auto-defer policy with authorization inside
the 1,750ms acknowledgement budget. Its source-level direct calls remain visible to the audit until
Task 6 replaces the transitional interaction facade with narrow responder calls. Every migrated
outbound operation must name its timeout, total budget, and retry policy.

## Migration Matrix

`Retry-safe?` describes the intended policy, not current automatic retry behavior. Except for the
representative `/create-character` runtime path, source call sites remain pending migration.
...
| Slash/context commands                                        | `src/commands/*.ts` (all 28 command files; 129 findings)                                                                                                                                                                                                  | Interaction replies, deferrals, edits, follow-ups, and modal acknowledgements                  | Yes                                                   | No callback retries                                                                     | Interaction responder                                                  | `/create-character` runtime wired in Task 5; source migration in Task 6 |
...
The initial audit found 11 direct `showModal()` calls. These routes require explicit modal-first
review because Discord does not permit deferring and then opening a modal:
...
`character_stat_select_menu.ts` contains two modal calls, producing 11 findings across 10 files.
Task 7 must record the selected restructuring pattern for each route before migration.
