...
selector-launch commands. The third batch migrates five non-modal inventory/RP commands while
leaving the modal-first edit routes isolated. Every migrated outbound operation must name its
timeout, total budget, and retry policy.
...
| Slash/context commands                                        | `src/commands/*.ts` (all 28 command files; 72 findings)                                                                                                                                                                                                   | Interaction replies, deferrals, edits, follow-ups, and modal acknowledgements                  | Yes                                                   | No callback retries                                                                     | Interaction responder                                                  | Core, character/game, and non-modal inventory/RP batches complete; modal RP, admin/support/subscription, and voice remain in Task 6 |
